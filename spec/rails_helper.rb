...
module TimeExtension
  def initialize(*args)
    if args.length == 0 && !Thread.current["__time_hack"]
      Thread.current["__time_hack"] = true
      time = TrackTimeStub.stubbed ? Time.now : Time.new
      new_sec = time.sec
      new_sec += Rational(time.usec, 1_000_000)
      super(
        time.year,
        time.mon,
        time.day,
      ...
      Thread.current["__time_hack"] = false
      time
    else
      super
    end
