...
RSpec.describe Post do
  ...
  describe "maximum media embeds" do
    ...
    let(:post_with_favicon) do
      post_with_body(
        '<img src="/images/favicons/discourse.png" class="favicon">',
        newuser
      )
    end
    ...
    context "with validation" do
      ...
      context "when posting links is limited to certain TL groups" do
        it "considers oneboxes links" do
          SiteSetting.post_links_allowed_groups =
        ...
        it "doesn't allow allow links if user is not in allowed groups" do
          SiteSetting.post_links_allowed_groups =
            Group::AUTO_GROUPS[:trust_level_2]
  ...
  describe "cooking" do
    ...
    it "passes the last_editor_id as the markdown user_id option" do
      ...
      PostAnalyzer
        ...
        .with(
          post.raw,
          { cook_method: Post.cook_methods[:regular], user_id: user_editor.id }
        )
      post.cook(post.raw)
    end
  ...
  describe "#rebake!" do
    ...
    it "uses inline onebox cache by default" do
      ...
      stub_request(:get, "http://testonebox.com/vvf").to_return(
        status: 200,
        body: <<~HTML
        <html><head>
    ...
    it "passing invalidate_oneboxes: true ignores inline onebox cache" do
      Jobs.run_immediately!
      stub_request(:get, "http://testonebox.com/vvf22").to_return(
        status: 200,
        body: <<~HTML
        <html><head>
          <title>hello this is Testonebox!</title>
  ...
  describe "#hide!" do
    ...
    it "should not run post validations" do
      ...
      expect { post.hide!(PostActionType.types[:off_topic]) }.to change {
        post.reload.hidden
      }.from(false).to(true)
    end
  ...
  describe "#unhide!" do
    ...
    it "should increase user_stat topic_count for first post" do
      ...
      expect do post.unhide! end.to change {
        post.user.user_stat.reload.topic_count
      }.from(0).to(1)
    end
    ...
    it "should decrease user_stat post_count" do
      ...
      expect do post_2.unhide! end.to change {
        post_2.user.user_stat.reload.post_count
      }.from(0).to(1)
    end
  end
  ...
  describe "uploads" do
    ...
    describe "#link_post_uploads" do
      it "finds all the uploads in the post" do
        ...
        expect(
          UploadReference.where(target: post).pluck(:upload_id)
        ).to contain_exactly(
          video_upload.id,
          video_upload_2.id,
          image_upload.id,
      ...
      it "cleans the reverse index up for the current post" do
        ...
        expect(post.reload.upload_references.pluck(:id)).to_not contain_exactly(
          post_uploads_ids
        )
      end
  ...
  describe "have_uploads" do
    it "should find all posts with the upload" do
      ...
      ids << Fabricate(
        :post,
        cooked:
          "A post with upload <img src='/#{upload_path}/1/defghijklmno.png'>"
      ).id
      ids << Fabricate(
        :post,
        cooked:
      ...
      ids << Fabricate(
        :post,
        cooked:
          "A post with upload <img src='/#{upload_path}/original/1X/abc/defghijklmno.png'>"
      ).id
      ids << Fabricate(
        :post,
        cooked:
  ...
  describe "#each_upload_url" do
    ...
    it "correctly identifies secure uploads" do
      ...
      # Test without domain:
      upload2_path =
        URI.parse(UrlHelper.cook_url(upload2.url, secure: true)).path
    ...
    it "skip S3 cdn urls with different path" do
      ...
      SiteSetting
        .Upload
        .stubs(:s3_cdn_url)
        .returns("https://cdn.example.com/site1")
  ...
  describe "#publish_changes_to_client!" do
    ...
    it "send message to all users participating in private conversation" do
      ...
      created_message =
        messages.select { |msg| msg.data[:type] == :created }.first
      expect(created_message).to be_present
      expect(created_message.data).to eq(message)
      expect(created_message.user_ids.sort).to eq(
      ...
      expect(stats_message.user_ids.sort).to eq(
        [user1.id, user2.id, user3.id].sort
      )
    end
    ...
    it "skips publishing topic stats when requested" do
      ...
      other_message =
        messages.select { |msg| msg.data[:type] == :anything }.first
  ...
  describe "#cannot_permanently_delete_reason" do
    ...
    it "returns nothing if different admin" do
      expect(post.cannot_permanently_delete_reason(Fabricate(:admin))).to eq(
        nil
      )
    end
  end
  ...
  describe "#canonical_url" do
    it "is able to determine correct canonical urls" do
      ...
      expect(post1.canonical_url).to eq(
        "#{topic_url}#post_#{post1.post_number}"
      )
      expect(post2.canonical_url).to eq(
        "#{topic_url}#post_#{post2.post_number}"
      )

      expect(post3.canonical_url).to eq(
        "#{topic_url}?page=2#post_#{post3.post_number}"
      )
      expect(post4.canonical_url).to eq(
        "#{topic_url}?page=2#post_#{post4.post_number}"
      )
    end
  end
  ...
  describe "public_posts_count_per_day" do
    ...
    it "collect closed interval public post count" do
      ...
      expect(
        Post.public_posts_count_per_day(2.days.ago, Time.now)
      ).not_to include(4.days.ago.to_date => 1)
    end
    ...
    it "returns the correct number of public posts per day when there are no public posts" do
      ...
      expect(
        Post.public_posts_count_per_day(10.days.ago, 5.days.ago)
      ).to be_empty
    end
    ...
    it "returns the correct number of public posts per day with category filter" do
      ...
      expect(
        Post.public_posts_count_per_day(10.days.ago, 5.days.ago, category.id)
